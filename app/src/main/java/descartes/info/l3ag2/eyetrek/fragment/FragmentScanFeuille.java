...
public class FragmentScanFeuille extends Fragment implements IOnBackPressed {

    SharedPreferences sharedPreferences;
    private static final String PREFS = "app_prefs";
    String insertUrl = "http://www.ens.math-info.univ-paris5.fr/~ij00084/saveLeaf.php";
    String scoreUrl = "http://www.ens.math-info.univ-paris5.fr/~ij00084/updateScoreLeaf.php";
    ...
    private ImageClassifierFpasF classifieurFpasF;
    ...
    // Informations au moment des résultats (type : "ce que vous avez pris en photo ne semble pas etre une feuille")
    private CardView cadre_information;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        BlurView blurView = view.findViewById(R.id.cadre_information);
        ...
        // Informations au moment des résultats (type : "ce que vous avez pris en photo ne semble pas etre une feuille")
        cadre_information = view.findViewById(R.id.cadre_information);
        cadre_information.setVisibility(View.GONE);
        ...
        */
        ...
        // On retourne la vue à l'activitée ScanFeuille
        return view;
    }
    ...
    private class ThreadResultats extends Thread {
        ...
        public void run(){
            ...
            //String textToShow = classifieurFpasF.classifyFrame(bitmap);
            ...
            if(NetworkUtils.isNetworkAvailable(getActivity()) && NetworkUtils.checkActiveInternetConnection(getActivity())) {
                new Thread(() -> {
                    ...
                    try {
                        ...
                        */
                        String reponse_serv = NetworkUtils.sendPost("http://172.30.46.155:5000/predict/", image_base64);// bufferedReader.readLine();
            ...
            //Log.e("ThreadResultats", classifieurFpasF.classifyFrame(bitmap));

            if(!erreur_reseau.get() || !local_erreur.get()){
                getActivity().runOnUiThread(new Runnable() {
                    //@SuppressLint("ResourceAsColor") ==> Fait put etre crasher l'appli surr certains tels
                    ...
                    public void run() {
                        ...
                        if(classifieurFpasF.getIsFeuilleValide()){
                            Log.w("ThreadResultats", "La feuille est valide");
                            //cadre_information.setVisibility(View.GONE);
                            cadre_information.setVisibility(View.VISIBLE);
                            informations.setText("Scrollez vers le bas pour voir plus de résultats");
                            informations.setTextColor(R.color.hightResult);
                            informations.setHintTextColor(R.color.hightResult);
                        } else {
                            Log.w("ThreadResultats", "La feuille n'est pas valide");
                            cadre_information.setVisibility(View.VISIBLE);
                            informations.setText("Ce que vous avez pris en photo ne semble pas être une feuille");
                            informations.setTextColor(R.color.lowResult);
                            informations.setHintTextColor(R.color.lowResult);
                        ...
                        StringRequest request = new StringRequest(Request.Method.POST, insertUrl, new com.android.volley.Response.Listener<String>() {
                            ...
                            protected Map<String, String> getParams() throws AuthFailureError {
                                ...
                                Double p = (Double) results.get(0).get(1);
                                parameters.put("id_user",id);
                                parameters.put("leaf",l);
                                parameters.put("percentage",p.toString());
                        ...
                        StringRequest request2 = new StringRequest(Request.Method.POST, scoreUrl, new com.android.volley.Response.Listener<String>() {
                            @Override
                            public void onResponse(String response) {
    ...
    private void set_infos_cartes(CardView carte, TextView titreCarte, TextView sureteCarte, TextView descriptionCarte, ImageView imageCarte, String resultat, Double surete){
        try{
            if(surete > 0.001){
                ArrayList<String> details_resultats = UtilitaireResultat.getInfo(resultat, getActivity());

                Log.e("set_infos_cartes", "UtilitaireResultat.getInfo() : " + details_resultats.toString());
    ...
    private void loadModel() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    classifieurFpasF = new ImageClassifierFpasF(getActivity());
                    classifieur = new ImageClassifier(getActivity(), UtilitaireModels.MODELE_FEUILLES_PATH, UtilitaireModels.LABELS_MODELE_FEUILLES_PATH);
                    ...
                    */
                    ...
                    e.printStackTrace();
                    if(classifieurFpasF == null){
                        throw new RuntimeException("Erreur pendant l'initialisation du classifieur ! (classifieurFpasF == null) ", e);
                    } else if(classifieur == null){
                        // Si le modele pour l'analyse de feuilles n'a pas pu etre chargé, on lui dit qu'il ne pourra
                        // faire d'analyse que s'il a du réseau
                        getActivity().runOnUiThread(new Runnable() {
    ...
    private void classifyFrameFpasF() {
        //afficher_validite("...", false);
        if (classifieurFpasF == null || getActivity() == null || camera == null) {
            //Log.e("classifyFrameFpasF()", "classifieurFpasF == null || getActivity() == null || camera == null");
            return;
        }
        Bitmap bitmap =
                surfaceCamera.getBitmap(ImageClassifierFpasF.DIM_IMG_SIZE_X, ImageClassifierFpasF.DIM_IMG_SIZE_Y);
        String textToShow = classifieurFpasF.classifyFrame(bitmap);
        boolean isFeuilleValide = classifieurFpasF.getIsFeuilleValide();
        bitmap.recycle();
